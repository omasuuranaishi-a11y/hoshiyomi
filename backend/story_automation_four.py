...
def run_story_slot(target_date:str|date|None=None,*,slot:str="morning",dry_run:bool=False,force:bool=False,generated_root:str|Path|None=None)->dict[str,Any]:
    ...
    hour={"morning":6,"noon":12,"evening":18,"night":21}[slot];facts=build_daily_sky(target,reading_hour=hour);content=build_slot_content(facts,slot);name=f"{slot}.jpg";path=generated/"story_assets"/target.isoformat()/name;render_slot_story(content,target,path)
