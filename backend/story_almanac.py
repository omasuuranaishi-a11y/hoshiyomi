...
def render_almanac(content:dict,day:date,path:str|Path)->Path:
    ...
    _center(d,f'{day.year}年{day.month}月{dday}日',158,_font(57,600),left=main_l,right=main_r)
    weekdays='月火水木金土日'; _center(d,f'{weekdays[day.weekday()]}曜日　｜　今日の月と星',244,_font(25,500,True),GRAY,main_l,main_r)
    d.line((main_c-237.5,300,main_c+237.5,300),fill=INK,width=1)

    # Left: phase now, plus one rotating almanac angle.
    d.rectangle((58,215,306,1590),outline=RULE,width=2)
    ...
    yy=rule_y+60
    for line in lines:
        yy=_center_block(d,line.format(**fmt),72,292,yy,_font(34,500,True),12)+28
    ...
    _center(d,'月のリズム',668,_font(27,500,True),GRAY,main_l,main_r)
    _center(d,f'月 は {moon}',722,_font(53,650),left=main_l,right=main_r)
    _center(d,f'{moon} {float(content["moon_degree"]):.1f}°を運行中',802,_font(29),GRAY,main_l,main_r);d.line((main_l,859,main_r,859),fill=INK,width=1)

    _center(d,'今日の星から',900,_font(37,650),left=main_l,right=main_r)
    ...
    _center_block(d,reading,main_l,main_r,962,_font(35,450,True),14);d.line((main_l,1154,main_r,1154),fill=RULE,width=1)
    _center(d,'今日のアスペクト',1186,_font(37,650),left=main_l,right=main_r)
    if aspect:
        ...
        for t,f in parts:d.text((x,1224),t,font=f,fill=INK,anchor='la');x+=d.textlength(t,font=f)
        _center(d,ASPECT_PLAIN.get(aname,f'{degree}度'),1289,_font(29,600,True),GRAY,main_l,main_r)
        ...
        _center(d,f'月は{moon}',1224,_font(37,600),left=main_l,right=main_r);expl='大きな主要アスペクトが少ない日。自分のペースを確かめて。'
    _center_block(d,expl,main_l,main_r,1338,_font(32,450,True),12);d.line((main_l,1514,main_r,1514),fill=RULE,width=1)
    _center(d,'今日どう過ごす？',1542,_font(37,650),left=main_l,right=main_r)
    action='返事や予定を一つに絞り、終わってから次へ進みましょう。'
    _center_block(d,action,main_l,main_r,1596,_font(35,450,True),13)
    # Instagramの下部UIに隠れないよう半円と文字を上へ移動。
    d.pieslice((main_c-350,1730,main_c+350,2210),180,360,fill=YELLOW)
    _center(d,'きょうの一言',1768,_font(27,650,True),left=main_l,right=main_r)
    _center(d,'星を知って、暮らしにひと工夫。',1818,_font(36,650),left=main_l,right=main_r)
    _center(d,'@omasu_horoscope',1872,_font(23,sans=True),left=main_l,right=main_r)
