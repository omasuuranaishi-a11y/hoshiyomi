...
8. Instagramストーリーへ投稿
9. 配色7種と装飾4種を組み合わせ、同じデザインを28日間繰り返さない
10. 同日の二重投稿を防ぎ、途中失敗時は未投稿分から再開
...
- 生成後、投稿前に必ずレイアウト検査を行います。
- 重要テキストはInstagramのプロフィール表示・返信欄に重ならない安全領域内だけに配置します。
- テキスト枠同士の重なり、枠からのはみ出し、画像サイズ・形式の異常がある場合は投稿せず停止します。
- 高品質な承認済みデザインを土台に、7配色×4装飾をローテーションします。
